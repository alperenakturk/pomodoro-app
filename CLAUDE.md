...
- **`src/lib/auth/AuthContext.jsx` + `src/hooks/useAuth.js`** — a second Context, structured identically to `LanguageContext`/`useTranslation` (same context-object/provider/hook file split, same reasoning: auth state is needed from deeply nested places like Settings' account row without prop-drilling). `AuthProvider` wraps `<App />` in `main.jsx`, outside `LanguageProvider` (no dependency either way). Exposes `{ user, loading, signInWithGoogle, signInWithEmail, signUpWithEmail, signOut }`; `user` is `null` for guests and `loading` is only ever true for the brief window before the initial `getSession()` call resolves. All four auth actions no-op with a `NOT_CONFIGURED_ERROR` when `supabase` is `null`.
...
- **`AuthModal.jsx`** — modeled on `DayReview.jsx`'s modal shell (fixed backdrop, centered card, focus moved in on open/restored on close) for visual consistency with the app's one other modal. Google button + email/password form (toggling between sign-in/sign-up) + a "Continue without an account" button that just calls `onClose` — dismissing it is a no-op with respect to app state, since guest mode was already the running mode.
- **`AccountStatus.jsx`** (Settings tab, first row in the Settings card) — "Signed in as `{{email}}`" + Sign out, or "Not signed in" + Sign in (opens `AuthModal`). Renders nothing while `loading` rather than flashing "Not signed in" before the initial session check resolves.

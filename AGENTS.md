...
- **Dev Mode (`src/lib/devMode.js`) is gated only by `import.meta.env.DEV`, not by account** — anyone who clones the repo and runs `npm run dev` can sign into their own real account and use its bypasses (instant-complete, achievement/streak preview) to write real, artificially-inflated achievement/streak data through the normal code paths. Accepted as low-priority: Supabase RLS (`user_id = auth.uid()`) confines this to the user's own account, and there's no leaderboard/friend-comparison feature yet for it to matter. Revisit (treat this data as untrusted without server-side validation) if one is ever built.
